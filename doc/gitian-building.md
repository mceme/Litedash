...
- [Building Dash Core](#building-litedash-core)
...
git clone https://github.com/dashpay/litedash
...
```bash
    ...
    From https://github.com/dashpay/litedash
...
and the litedash git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../litedash/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/litedash
SIGPATH=/some/root/path/litedash-detached-sigs

./bin/gbuild --url litedash=${BTCPATH},signature=${SIGPATH} ../litedash/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/litedash-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/litedash-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/litedash-osx-build.assert
```
...
[litedash/gitian.sigs](https://github.com/dashpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
