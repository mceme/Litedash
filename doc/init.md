...
see `contrib/debian/examples/litedash.conf`.
...
Configuration file:  `/etc/dashcore/litedash.conf`  
...
dashcore user and group.  Access to litedash-cli and other dashd rpc clients
can then be controlled by group membership.

3b) Mac OS X

Binary:              `/usr/local/bin/dashd`  
Configuration file:  `~/Library/Application Support/DashCore/litedash.conf`  
...
Copy org.litedash.dashd.plist into ~/Library/LaunchAgents. Load the launch agent by
running `launchctl load ~/Library/LaunchAgents/org.litedash.dashd.plist`.
...
You will need to modify org.litedash.dashd.plist if you intend to use it as a
